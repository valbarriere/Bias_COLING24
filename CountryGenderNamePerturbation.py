"""
Script transforming the data into perturbed data with names typical from other
countries, in order to test the bias of the model
...
class PerturbedExamples:
    ...
    def all_countries(self, texts, labels=None, n=2):
        """
        ...
        for country, gender in itertools.product(
            self.list_countries, ["male"] + ["female"] * self.use_female
        ):
            ret = Perturb.perturb(
                pdata, Perturb.change_names_country_specific(country, gender), n=n
            )
            list_ex_to_tag = []
            list_ex_to_tag_ini = []
            ...
            list_transformed[country][gender] = (
                [kk for k in list_ex_to_tag for kk in k[0]],
                [kk for k in list_ex_to_tag for kk in k[1]],
            )

            # on sauve les tweeets d'origine
            if "Original" not in list_transformed.keys():
                list_transformed["Original"] = (
                    [k[0] for k in list_ex_to_tag_ini],
                    [k[1] for k in list_ex_to_tag_ini],
                )
...
if __name__ == "__main__":
    ...
    print(
        perturber.all_countries(
            data, ["positive", "positive", "negative", "negative", "neutral"]
        )
    )
